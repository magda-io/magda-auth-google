...
>
> If you provide your own PostgreSQL account with a **non-default** privileged username, a self-managed `existingSecret` must also carry the `postgresql-postgres-password` key, otherwise the database pod fails with `CreateContainerConfigError`. `postgresqlUsername` cannot be changed after the database's first boot.
...
- name: magda-auth-google
  # Magda v7+: use the latest v4.x. Magda v6 or lower: use the latest v3.x. See "Version Compatibility" above.
  version: "4.0.0"
